...
# ── Title ─────────────────────────────────────────────────────────────────────
lbl(9, 9.77, "saas-assurance — Reference Architecture (1/3): System Overview",
    sz=12, bold=True, c=C_BD)
lbl(9, 9.48,
    "Read-only  ·  JWT Bearer (SFDC)  ·  OAuth 2.0 (Workday)"
    "  ·  OWASP Agentic App Top 10  ·  10 agents  ·  7 skills",
    sz=7.5, c=C_GR)
...
for i, t in enumerate(["HCM / Finance", "OAuth 2.0 Client Credentials",
                        "REST API v1 · RaaS (custom reports)", "Manual questionnaire"]):
...
for i, t in enumerate(["Any edition", "JWT Bearer Flow",
                        "REST + Tooling + Metadata API", "SecuritySettings SOQL"]):
    lbl(15.4, 8.2 - i * 0.33, t, sz=6.8, c=C_GR)

# ── Agent Layer (center-left, summary) ───────────────────────────────────────
sec(0.3, 1.1, 8.5, 4.55, "Agent Layer  (OpenAI API  ·  gpt-5.3-chat-latest)",
    C_BL, C_BD, C_BD)
...
lbl(4.5, 1.87, "Security Harness  (harness/loop.py · tools.py)",
    sz=7, bold=True, c=C_A)
lbl(4.5, 1.52,
    "_TOOL_REQUIRES gate  ·  memory guard  ·  _sanitize_org / _safe_inp_path"
    "  ·  audit.jsonl  ·  OWASP A1–A9",
    sz=6.2, c=C_GR)
lbl(4.5, 2.31, "→ See Architecture (2/3) for full agent detail",
    sz=6.5, c=C_BM)
...
lbl(6.05, 0.72, "Generated Artifacts  (docs/oscal-salesforce-poc/generated/<org>/<date>/)",
    sz=7.0, bold=True, c=C_GD)
lbl(6.05, 0.38,
...
lbl(14.9, 0.38,
    "docker compose up -d  →  OpenSearch · 3 dashboards · 40 panels"
    "  →  export_to_opensearch.py",
    sz=6.0, c=C_GR)
