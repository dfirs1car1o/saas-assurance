...
# ── Title ─────────────────────────────────────────────────────────────────────
lbl(10, 8.77, "saas-assurance — Reference Architecture (3/3): Data Pipeline", sz=12, bold=True, c=C_BD)
lbl(
    10,
    8.47,
...
for i, (phase_lbl, skill, artifact) in enumerate(phases):
    ...
    # arrow to next
    if i < 6:
        arr(px + PH_W, SFDC_PHASE_Y + PH_H / 2, phase_x[i + 1], SFDC_PHASE_Y + PH_H / 2, c=C_T, lw=1.1)
...
for i, (phase_lbl, skill, artifact) in enumerate(phases):
    ...
    # arrow to next
    if i < 6:
        arr(px + PH_W, WD_PHASE_Y + PH_H / 2, phase_x[i + 1], WD_PHASE_Y + PH_H / 2, c="#5C6BC0", lw=1.1)

# vertical divider between swim lanes
ax.plot([0.25, 19.75], [SFDC_LANE_Y, SFDC_LANE_Y], color=C_GR, lw=0.8, linestyle="--", zorder=3)
...
lbl(17.38, 0.3, "gen_poam.py  ·  gen_ssp.py\nassessment_results.py  →  OSCAL 1.1.2", sz=6.2, c=C_P)
