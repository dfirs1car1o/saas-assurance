...
def label(x, y, text, size=8, color="black", bold=False, ha="center", va="center", wrap=False, zorder=5):
    ...
    ax.text(x, y, text, fontsize=size, color=color, fontweight=weight,
            ha=ha, va=va, zorder=zorder, fontfamily=FONT_MAIN,
...
def section(x, y, w, h, title, fc, ec, title_color, title_size=8.5):
    ...
    label(x + w / 2, y + h - 0.22, title, size=title_size,
          color=title_color, bold=True, zorder=3)
...
# ── Title ─────────────────────────────────────────────────────────────────────
label(9, TOP - 0.05, "saas-posture — Reference Architecture",
      size=13, bold=True, color=C_BLUE_DARK)
label(9, TOP - 0.42, "Read-only · JWT Bearer (Salesforce) · OAuth 2.0 (Workday) · No SOAP",
      size=8, color=C_GREY)
...
section(SEC1_X, SEC1_Y, SEC1_W, SEC1_H,
        "SaaS Platforms  (read-only)", C_ORANGE_LIGHT, C_ORANGE, C_ORANGE, 8.5)

# Workday box
platform_box(SEC1_X + 0.2, SEC1_Y + 0.3, 2.8, 3.0,
             "Workday Tenant",
             ["HCM / Finance",
...
# Salesforce box
platform_box(SEC1_X + 3.3, SEC1_Y + 0.3, 2.7, 3.0,
             "Salesforce Org",
             ["Any edition",
              "JWT Bearer Flow",
              "Tooling API",
...
section(SEC2_X, SEC2_Y, SEC2_W, SEC2_H,
        "OSCAL Config  (config/)", C_PURPLE_LIGHT, C_PURPLE, C_PURPLE, 8.5)
...
for text, cx in config_items:
    ...
    label(SEC2_X + cx - 1.0 + 1.05, SEC2_Y + 0.35 + 0.57, text,
          size=6.8, color=C_PURPLE, bold=False)

# Component-definitions note
label(SEC2_X + 5.35, SEC2_Y + 1.75,
      "config/component-definitions/  ←  per-control evidence spec (API query + method)",
      size=6.5, color=C_GREY, ha="center")

# ─────────────────────────────────────────────────────────────────────────────
# SECTION 3 — Agent Layer
# ─────────────────────────────────────────────────────────────────────────────
SEC3_X, SEC3_Y, SEC3_W, SEC3_H = 0.3, 6.8, 11.0, 14.0
section(SEC3_X, SEC3_Y, SEC3_W, SEC3_H,
        "Agent Layer  (OpenAI API  ·  gpt-5.3-chat-latest)", C_BLUE_LIGHT, C_BLUE_DARK, C_BLUE_DARK, 8.5)

# Orchestrator — centrepiece
ORC_X, ORC_Y, ORC_W, ORC_H = 0.65, 13.2, 3.0, 1.1
agent_box(ORC_X, ORC_Y, ORC_W, ORC_H, "Orchestrator", "plans + dispatches",
          fc="#BBDEFB", ec=C_BLUE_DARK)
...
for name, sub, y_pos in sub_agents:
    ...
    ax.annotate("", xy=(4.6, y_pos - 0.07), xytext=(ORC_X + ORC_W, ORC_Y + ORC_H / 2),
...
section(SEC4_X, SEC4_Y, SEC4_W, SEC4_H,
        "Skills  (Python CLIs  ·  read-only)", C_TEAL_LIGHT, C_TEAL, C_TEAL, 8.5)
...
section(SEC5_X, SEC5_Y, SEC5_W, SEC5_H,
        "Generated Artifacts  (docs/oscal-salesforce-poc/generated/)",
        C_GREEN_LIGHT, C_GREEN_DARK, C_GREEN_DARK, 8.5)
...
section(SEC6_X, SEC6_Y, SEC6_W, SEC6_H,
        "Continuous Monitoring  (optional)", C_GREEN_LIGHT, C_GREEN_DARK, C_GREEN_DARK, 8.5)
...
label(SEC6_X + 3.05, SEC6_Y + 1.22,
      "--auto --org <alias> --date <YYYY-MM-DD>", size=6.8, color=C_GREY)
label(SEC6_X + 3.05, SEC6_Y + 0.90,
      "Indexes sscf-findings-* + sscf-runs-*", size=6.8, color=C_GREY)
label(SEC6_X + 3.05, SEC6_Y + 0.57,
      "Score trend · POA&M · Domain risk · Severity", size=6.8, color=C_GREY)
...
arrow(SEC4_X + 2.75, 19.4 - 0.04,
      SEC1_X + 1.6, SEC1_Y,
      color=C_ORANGE, lw=1.2)   # workday-connect → Workday
arrow(SEC4_X + 2.75, 17.5 - 0.04,
      SEC1_X + 4.65, SEC1_Y,
      color=C_ORANGE, lw=1.2)   # sfdc-connect → Salesforce

# Config → Assessor / Collector (left)
arrow(SEC2_X + 3.0, SEC2_Y,
      SEC3_X + 1.65, SEC3_Y + SEC3_H,
      color=C_PURPLE, lw=1.1, style="->,head_width=0.15,head_length=0.1")

# Artifacts → report-gen → Governance (right side of artifacts)
arrow(SEC5_X + SEC5_W, SEC5_Y + SEC5_H / 2,
      SEC6_X, SEC6_Y + SEC6_H / 2,
      color=C_GREEN_DARK, lw=1.2)

# Skills → artifacts (downward)
arrow(SEC4_X + SEC4_W / 2, SEC4_Y,
      SEC5_X + SEC5_W * 0.75, SEC5_Y + SEC5_H,
      color=C_TEAL, lw=1.2)
...
# ─────────────────────────────────────────────────────────────────────────────
chain_y = 0.95
ax.add_patch(FancyBboxPatch((0.3, 0.35), 17.4, 0.85,
             boxstyle="round,pad=0,rounding_size=0.15",
             facecolor="#E8EAF6", edgecolor="#3949AB", lw=0.9, zorder=1))
label(9, chain_y,
      "Framework chain:  Platform control  →  SSCF domain  →  CCM v4.1  →  ISO 27001:2022 Annex A  →  SOX / HIPAA / SOC2 / NIST 800-53 / PCI DSS / GDPR",
      size=6.8, color="#283593", bold=False)
