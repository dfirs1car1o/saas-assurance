...
def arr(x0, y0, x1, y1, c=C_GR, lw=1.2, style="->", dash=False):
    ...
    ax.annotate("", xy=(x1, y1), xytext=(x0, y0),
                arrowprops={"arrowstyle": style, "color": c, "lw": lw,
                            "linestyle": ls,
...
# ── Title ─────────────────────────────────────────────────────────────────────
lbl(8, 13.77, "saas-assurance — Reference Architecture (2/3): Agent Architecture",
...
sec(0.3, 0.3, 15.4, 13.0,
    "Agent Layer  (harness/loop.py  ·  OpenAI tool_use ReAct)", C_BL, C_BD, C_BD)
...
for idx, (name, sub, is_strict) in enumerate(agents):
    ...
    lbl(cx, cy + 0.32, name, sz=8, bold=True,
        c=C_RD if is_strict else C_BD)
    ...
    ax.annotate("", xy=(cx, by + 1.5),
                xytext=(8.0, ORC_BOT),
                arrowprops={
                    "arrowstyle": "->,head_width=0.15,head_length=0.1",
                    "color": C_BM, "lw": 0.7, "linestyle": "dashed",
                    "connectionstyle": "arc3,rad=0.0",
                }, zorder=3)
...
lbl(7.85, 1.68, "Security Controls Harness  (harness/loop.py · harness/tools.py)",
    sz=8, bold=True, c=C_A)
