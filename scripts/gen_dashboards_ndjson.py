...
def _viz(id_: str, title: str, vis_state: dict, query: str = "",
         index_id: str = "sscf-findings-*", desc: str = "") -> dict:
    ...
    return _viz(id_, title, {
        ...
        "params": {
            ...
            "metric": {
                ...
                "colorsRange": [{"from": 0, "to": 0.5},
                                {"from": 0.5, "to": 0.75},
                                {"from": 0.75, "to": 1.0}],
                "labels": {"show": True}, "invertColors": True,
                "style": {"bgFill": "#000", "bgColor": True,
                          "labelColor": False, "subText": subtext, "fontSize": 48},
            },
        },
        "aggs": [{"id": "1", "enabled": True, "type": "avg", "schema": "metric",
                  "params": {"field": "overall_score"}}],
    }, query=f"platform : {platform}", index_id="sscf-runs-*",
...
def donut_pie(id_: str, title: str, field: str, query: str = "",
              index_id: str = "sscf-findings-*", desc: str = "") -> dict:
    return _viz(id_, title, {
        "type": "pie",
        "params": {
    ...
    }, query=query, index_id=index_id, desc=desc)


def hbar(id_: str, title: str, term_field: str, query: str = "",
         split_field: str | None = None, index_id: str = "sscf-findings-*",
         desc: str = "") -> dict:
    """Horizontal bar (terms on Y-axis) — good for long control names."""
    aggs: list[dict] = [
        {"id": "1", "enabled": True, "type": "count", "schema": "metric", "params": {}},
        {"id": "2", "enabled": True, "type": "terms", "schema": "segment",
         "params": {"field": term_field, "size": 10, "order": "desc", "orderBy": "1",
                    "otherBucket": False, "missingBucket": False, "customLabel": "Control"}},
    ]
    if split_field:
        aggs.append({
            "id": "3", "enabled": True, "type": "terms", "schema": "group",
            "params": {"field": split_field, "size": 5, "order": "desc", "orderBy": "1",
                       "otherBucket": False, "missingBucket": False, "customLabel": split_field.split(".")[0].title()},
        })
    return _viz(id_, title, {
        "type": "histogram",
        "params": {
            ...
            "categoryAxes": [{"id": "CategoryAxis-1", "type": "category", "position": "left",
                               ...
                               "labels": {"show": True, "filter": False, "truncate": 250}, "title": {}}],
...
def vbar(id_: str, title: str, term_field: str, query: str = "",
         split_field: str | None = None, index_id: str = "sscf-findings-*",
         desc: str = "") -> dict:
...
def line_trend(id_: str, title: str, query: str = "",
               index_id: str = "sscf-runs-*", split_field: str | None = None) -> dict:
    """Score trend line chart."""
    aggs: list[dict] = [
        {"id": "1", "enabled": True, "type": "avg", "schema": "metric",
         "params": {"field": "overall_score"}},
        {"id": "2", "enabled": True, "type": "date_histogram", "schema": "segment",
         "params": {"field": "generated_at_utc", "useNormalizedEsInterval": True,
    ...
    if split_field:
        aggs.append({
            "id": "3", "enabled": True, "type": "terms", "schema": "group",
            "params": {"field": split_field, "size": 5, "order": "desc", "orderBy": "1",
                       "otherBucket": False, "missingBucket": False, "customLabel": "Platform"},
        })
    return _viz(id_, title, {
        "type": "line",
        "params": {
        ...
        "aggs": aggs,
    }, query=query, index_id=index_id)


def agg_table(id_: str, title: str, buckets: list[tuple[str, str, int]],
              query: str = "", index_id: str = "sscf-findings-*", desc: str = "") -> dict:
    aggs: list[dict] = [
        {"id": "1", "enabled": True, "type": "count", "schema": "metric", "params": {}},
    ]
    for i, (field, label, size) in enumerate(buckets, start=2):
        aggs.append({
            ...
            "params": {"field": field, "size": size, "order": "desc", "orderBy": "1",
                       "otherBucket": False, "missingBucket": False, "customLabel": label},
        })
    return _viz(id_, title, {
        "type": "table",
        "params": {
...
def saved_search(id_: str, title: str, columns: list[str], query: str = "",
                 index_id: str = "sscf-findings-*", desc: str = "") -> dict:
    return {
        "type": "search", "id": id_,
        "attributes": {
            "title": title, "description": desc, "hits": 0,
            "columns": columns,
            "sort": [["severity", "desc"]],
            "version": 1,
            "kibanaSavedObjectMeta": {"searchSourceJSON": _src(query, index_id)},
...
def panel(idx: int, x: int, y: int, w: int, h: int,
          obj_id: str, obj_type: str = "visualization") -> dict:
    return {
        "panelIndex": str(idx), "gridData": {"x": x, "y": y, "w": w, "h": h, "i": str(idx)},
        "id": obj_id, "type": obj_type, "version": "2.19.0",
    }
...
def dashboard_obj(id_: str, title: str, desc: str,
                  panels: list[dict], refs: list[dict]) -> dict:
    return {
        "type": "dashboard", "id": id_,
        "attributes": {
            "title": title, "hits": 0, "description": desc,
            "panelsJSON": _j(panels),
            "optionsJSON": _j({"useMargins": True, "hidePanelTitles": False}),
            "version": 1,
            "timeRestore": True, "timeTo": "now", "timeFrom": "now-1y",
...
DETAIL_COLS = ["control_id", "sbs_title", "domain", "severity",
               "status", "poam_status", "owner", "due_date", "remediation"]
...
OBJECTS: list[dict] = [
    ...
    donut_pie("viz-platform-pie", "Findings by Platform", "platform",
              desc="Finding count split by platform"),
    line_trend("viz-combined-score-trend", "Score Trend — Both Platforms",
               split_field="platform"),
    vbar("viz-combined-domain-bar", "All Platforms — Findings by Domain",
         "domain", query="status : fail OR status : partial", split_field="platform",
         desc="Fail/partial findings per domain, split by platform"),
    vbar("viz-combined-severity-bar", "All Platforms — Findings by Severity",
         "severity", split_field="platform",
         desc="Severity distribution split by platform"),
    count_tile("viz-combined-open-poam", "Open POA&M (All Platforms)", "Open Items",
               query="poam_status : Open"),

    # ── Salesforce-specific ───────────────────────────────────────────────────
    count_tile("viz-sfdc-pass-count",     "Controls Passing",    "Passing",
               query="platform : salesforce AND status : pass"),
    count_tile("viz-sfdc-fail-count",     "Controls Failing",    "Failing",
               query="platform : salesforce AND status : fail"),
    count_tile("viz-sfdc-critical-count", "Critical Failures",   "Critical",
               query="platform : salesforce AND status : fail AND severity : critical"),
    count_tile("viz-sfdc-open-poam",      "Open POA&M Items",    "Open Items",
               query="platform : salesforce AND poam_status : Open"),

    donut_pie("viz-sfdc-status-pie", "Salesforce — Control Status",
              "status", query="platform : salesforce",
              desc="Pass / Fail / Partial / Not Applicable distribution"),

    hbar("viz-sfdc-top-failing-bar", "Salesforce — Top Failing Controls",
         "sbs_title.keyword",
    ...
    vbar("viz-sfdc-domain-bar", "Salesforce — Risk by Domain",
         "domain",
         query="platform : salesforce AND (status : fail OR status : partial)",
         split_field="status",
         desc="Fail/partial findings per SSCF domain, stacked by status"),

    vbar("viz-sfdc-severity-bar", "Salesforce — Findings by Severity",
         "severity", query="platform : salesforce", split_field="status",
         desc="Severity distribution stacked by control status"),

    hbar("viz-sfdc-owner-bar", "Salesforce — Open Items by Owner",
         "owner.keyword",
         query="platform : salesforce AND (status : fail OR status : partial)",
         desc="Which owner/team carries the most open remediation items"),

    line_trend("viz-sfdc-score-trend", "Salesforce — Score Over Time",
               query="platform : salesforce"),

    agg_table("viz-sfdc-critical-table", "Salesforce — Critical & High Failures",
              buckets=[("severity", "Severity", 5), ("control_id", "Control ID", 10),
                       ("sbs_title.keyword", "Description", 1), ("domain", "Domain", 1)],
              query="platform : salesforce AND (status : fail OR status : partial)"
                    " AND (severity : critical OR severity : high)"),

    agg_table("viz-sfdc-poam-table", "Salesforce — POA&M Open Items",
              buckets=[("control_id", "Control", 50), ("severity", "Severity", 5),
                       ("poam_status", "Status", 5)],
              query='platform : salesforce AND (poam_status : Open OR poam_status : "In Progress")'),

    # ── Workday-specific ──────────────────────────────────────────────────────
    count_tile("viz-wd-pass-count",     "Controls Passing",    "Passing",
               query="platform : workday AND status : pass"),
    count_tile("viz-wd-fail-count",     "Controls Failing",    "Failing",
               query="platform : workday AND status : fail"),
    count_tile("viz-wd-critical-count", "Critical Failures",   "Critical",
               query="platform : workday AND status : fail AND severity : critical"),
    count_tile("viz-wd-open-poam",      "Open POA&M Items",    "Open Items",
               query="platform : workday AND poam_status : Open"),

    donut_pie("viz-wd-status-pie", "Workday — Control Status",
              "status", query="platform : workday",
              desc="Pass / Fail / Partial / Not Applicable distribution"),

    hbar("viz-wd-top-failing-bar", "Workday — Top Failing Controls",
         "sbs_title.keyword",
    ...
    vbar("viz-wd-domain-bar", "Workday — Risk by Domain",
         "domain",
         query="platform : workday AND (status : fail OR status : partial)",
         split_field="status",
         desc="Fail/partial findings per SSCF domain, stacked by status"),

    vbar("viz-wd-severity-bar", "Workday — Findings by Severity",
         "severity", query="platform : workday", split_field="status",
         desc="Severity distribution stacked by control status"),

    hbar("viz-wd-owner-bar", "Workday — Open Items by Owner",
         "owner.keyword",
         query="platform : workday AND (status : fail OR status : partial)",
         desc="Which owner/team carries the most open Workday remediation items"),

    line_trend("viz-wd-score-trend", "Workday — Score Over Time",
               query="platform : workday"),

    agg_table("viz-wd-critical-table", "Workday — Critical & High Failures",
              buckets=[("severity", "Severity", 5), ("control_id", "Control ID", 10),
                       ("sbs_title.keyword", "Description", 1), ("domain", "Domain", 1)],
              query="platform : workday AND (status : fail OR status : partial)"
                    " AND (severity : critical OR severity : high)"),

    agg_table("viz-wd-poam-table", "Workday — POA&M Open Items",
              buckets=[("control_id", "Control", 50), ("severity", "Severity", 5),
                       ("poam_status", "Status", 5)],
              query='platform : workday AND (poam_status : Open OR poam_status : "In Progress")'),

    # ── Saved searches ────────────────────────────────────────────────────────
    saved_search("search-sfdc-failing",
                 "Salesforce — Failing Controls (Document View)", DETAIL_COLS,
                 query="platform : salesforce AND (status : fail OR status : partial)",
                 desc="Row-level Salesforce fail/partial findings"),
    saved_search("search-wd-failing",
                 "Workday — Failing Controls (Document View)", DETAIL_COLS,
                 query="platform : workday AND (status : fail OR status : partial)",
                 desc="Row-level Workday fail/partial findings"),
    saved_search("search-all-failing",
                 "All Platforms — Failing Controls (Document View)", ALL_COLS,
...
def _platform_dashboard(plat: str, score_id: str,
                         pass_id: str, fail_id: str, crit_count_id: str, poam_count_id: str,
                         top_failing_id: str, status_pie_id: str,
                         domain_id: str, sev_id: str,
                         owner_id: str, trend_id: str,
                         crit_table_id: str, poam_table_id: str,
                         search_id: str,
                         dash_id: str, dash_title: str, dash_desc: str) -> dict:
    panels = [
        # Row 0
...
sfdc_dash = _platform_dashboard(
    ...
    crit_count_id="viz-sfdc-critical-count",
    poam_count_id="viz-sfdc-open-poam",
...
wd_dash = _platform_dashboard(
    ...
    crit_count_id="viz-wd-critical-count",
    poam_count_id="viz-wd-open-poam",
...
OBJECTS.append(
    hbar("viz-combined-top-failing-bar", "All Platforms — Top Failing Controls",
         "sbs_title.keyword",
         query="status : fail OR status : partial",
         split_field="platform",
